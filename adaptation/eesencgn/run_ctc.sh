...
if [ $stage -le 1 ]; then
  ...
  echo =====================================================================
  local/cgn_data_prep.py /pylon5/ci560op/fciannel/cgn/wav/ /pylon5/ci560op/fciannel/cgn/annot/ $datadir
  cp resources/units.txt data/lang_phn
  python local/cgn_text_to_lexicon.py
  mv data/totaldata/textl data/totaldata/text

#  local/mscoco_data_prep.sh $audio_dir $train_list $val_list || exit 1;
