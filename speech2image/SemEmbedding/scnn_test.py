...
PRINT_SIM = True
LAST_BATCH = (n_start-n_end+1)%batch_size
...
def scnn_test(X_test, Z_test_vgg, ntop):#(ntx, sp_test, im_test, ntop):
    ...
    data = np.load('scnn_pmtrs.npz', encoding='ASCII')
    pmtrs = data['arr_0']
    _w_in = pmtrs[0]
    _b_in = pmtrs[1]
    _w_hidden1 = pmtrs[2]
    ...
    if PRINT_MODEL:
        ...
        print(_w_hidden2)
        print('second hidden bias:', _b_hidden2.shape)
        print(_b_hidden2)
        print('vgg embed weight:', _w_embed.shape)
        print(_w_embed)
        print('vgg embed bias:', _b_embed.shape)
        print(_b_embed)
    
    for k in range(nbatch):
        X_batch = X_test_4d[batch_size*k:batch_size*(k+1)]
        Z_batch = Z_test_vgg[batch_size*k:batch_size*(k+1)]
        #print('first hidden layer output:', _h1.shape)
        #print(_h1)
        _X_zm = sess.run(X_zm, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
        #print('Input after mean subtraction:', _X_zm[200:800])
        ...
        if PRINT_HIDDEN_OUT:
            ...
            print(_X_zm[:, :, 200:800])
            _h1 = sess.run(h1, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
            print('first hidden layer:', _h1)
            _h2 = sess.run(h2, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
            print('second hidden layer:', _h2)
            _h3 = sess.run(h3, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
            print('third hidden:', _h3)
            _h4_re = sess.run(h4_re, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
            print('fourth hidden:', _h4_re)
            _h4_ren = sess.run(h4_ren, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
            print('fourth hidden:', _h4_ren)
        
        #print('VGG output:', Z_curr_vgg)
        #print('Shape:', Z_curr_vgg.shape, '\n')
        #print(Z_curr_vgg, '\n')
        ...
        # For test purpose
        similarity_raw = sess.run(s_a, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
        if PRINT_SIM:
            print('Similarity_raw from retrieval by extracting the simialrity', similarity_raw)
        similarity = sess.run(s, feed_dict={w_in:_w_in, b_in:_b_in, w_hidden1:_w_hidden1, b_hidden1:_b_hidden1, w_hidden2:_w_hidden2, b_hidden2:_b_hidden2, w_out:_w_out, b_out:_b_out, w_embed:_w_embed, b_embed:_b_embed, X_in:X_batch, Z_penult_vgg:Z_batch})
    if LAST_BATCH:
    ...
    #print('Embedding vector of speech:', _Z_embed_sp)
    #print('Embedding vector of image:', _Z_embed_vgg)
    similarity_raw = np.dot(_Z_embed_sp, np.transpose(_Z_embed_vgg))
    similarity = np.maximum(similarity_raw, np.zeros((ntx, ntx)))
    # Similarity for annotation
    similarity_ann_raw = np.dot(_Z_embed_vgg, np.transpose(_Z_embed_sp))
    similarity_ann = np.maximum(similarity_ann_raw, np.zeros((ntx, ntx)))

    #print('Similarity_raw from retrieval by extracting the vectors:', similarity_raw)
    ...
    if PRINT_SIM:
        print('Raw similarity score for retrieval:', similarity_raw)
        print('Raw similarity score for annotation:', similarity_ann_raw)
...
X_test = data['arr_0'][n_start:n_end]
data_im = np.load(im_test)
Z_test_vgg = data_im['arr_0'][n_start:n_end]
