...
def count_types(qsp, qim, dict_im2tx, isel):
    qim_sel = [qim[j] for j in isel]

    show_im = 0
    impath = 'Flicker8k_Dataset/'

    # show the selected images, and print the captions by looking up the dictionary
    if show_im:
    ...
    return len(dict_imtype)

'''def find_overlap(idices1, idices2):
    n_overlap = 0
    dict1 = find_types(idices1)
    dict2 = find_types(idices2)'''

#Find the indices of image/caption pair in the list to show
# Read a dictionary to map image file to text captions
wavpath = 'flickr_audio/wav2capt.txt'
dict_im2tx = read_captions()
    
# Read lists of image files and audio captions
qsp = []
qim = []
...
if int(sys.argv[1]) == 1:
    ...
    # show the indices in the closed interval specified by user
    if int(sys.argv[1]) == 0:
...
ndict = count_types(qsp, qim, dict_im2tx, isel)
